...
GELU_PLUGIN_LIBRARY = os.path.join(WORKING_DIR, 'build/libgeluplugin.so')

# Path to which trained model will be saved (check README.md)
MODEL_PATH = os.path.join(WORKING_DIR, 'models/frozen_graph.pb')
...
class ModelData(object):
    ...
    Gelu_NAME = "sequential/Gelu/Gelu/Erf"
...
def prepare_namespace_plugin_map():
    ...
    trt_gelu = gs.create_plugin_node(name="trt_gelu", op="CustomGeluPlugin")
    namespace_plugin_map = {ModelData.Gelu_NAME: trt_gelu}
...
def model_to_uff(model_path):
    ...
    plugin_map = prepare_namespace_plugin_map()
    ...
    uff.from_tensorflow(dynamic_graph.as_graph_def(), [ModelData.OUTPUT_NAME],
                        ...
                        text=True)
    with tf.io.gfile.GFile('models/converted_frozen.pb', 'wb') as f:
        f.write(dynamic_graph.as_graph_def().SerializeToString())
    return output_uff_path


# Builds TensorRT Engine
def build_engine(model_path, is_save=True):
...
def main():
    ...
    if not os.path.isfile(GELU_PLUGIN_LIBRARY):
        raise IOError("\n{}\n{}\n{}\n".format(
            "Failed to load library ({}).".format(GELU_PLUGIN_LIBRARY),
        ...
        with engine.create_execution_context() as context:
            ...
            for i in range(num):
                ...
                [pred] = common.do_inference(context,
                                             ...
                                             outputs=outputs,
                                             stream=stream)
                print("Origin value:{} Prediction: {}".format(
                    test_case, np.argmax(pred)))
                count += 1
                if np.argmax(pred) == test_case:
                    acc_num += 1
            print("Test {} number,accuracy:{}".format(count, acc_num / count))
